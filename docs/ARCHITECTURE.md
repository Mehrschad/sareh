...
- **هفت گونه از دوازده گونه‌ی تمرین** هنوز نیامده‌اند. موتور آماده‌ی آنهاست:
  یک زیرگونه‌ی `Question` و یک ویجت، و `_build` یک شاخه‌ی تازه می‌گیرد.
  افزودنِ «ریشه‌یاب» دقیقاً همین سه تکه بود و هیچ فایلِ دیگری را دست نزد —
  جز `lesson_page`، که چون `Question` یک `sealed class` است، کامپایلر خودش
  جای نبودنِ شاخه را نشان داد.
- **بسته‌های نمادِ دوم و سوم** («کهن» و «هندسی») هنوز نیامده‌اند؛ تنها
  «اسطوره» هست.
- **`answers_match` هنوز صدا زده نمی‌شود** — گونه‌های تمرینِ کنونی همه
  گزینه‌ای‌اند و تایپ ندارند. با آمدنِ «نویسش» به کار می‌آید.
- **صدا** هنوز نیست.
